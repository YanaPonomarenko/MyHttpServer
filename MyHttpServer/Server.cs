...
class EmailService
{
    public void SendEmail(string toEmail, string login)
    {
        try
        {
            ...
            MailMessage message = new MailMessage("test@localhost.com", toEmail);
            message.Subject = "Реєстрація успішна";
            message.Body = $"Вітаємо, {login}! Ви успішно зареєструвалися.";
